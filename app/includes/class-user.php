...
class User
{
    ...
    public function get_available_current_cities(): array
    {
        global $db;

        $query = $db->query("select distinct user_current_city from users where user_current_city is not null and trim(user_current_city) <> '';");
        $result = $query->fetch_all(MYSQLI_ASSOC);

        return array_column($result, 'user_current_city');
    }
     ...
     * @return array
     */
    public function search($query, $tab, $offset = 0, $options = [])
    {
        global $db, $system;
        $results = [];
        if ($tab != "posts") {
            ...
            case 'users':
                ...
                if (!empty($options)) {
                    foreach ($options as $key => &$option) {
                        $option = "user_{$key} = " . secure($option);
                    }

                    $extraWhere = 'AND ' . implode(' AND ', $options);
                }
    ...
    private function fund(string $entity, int $id, float $amount): array
    {
        ...
        $query = $db->query(sprintf('select user_relysia_address as address from users where user_id = %1$s limit 1', secure($userId, 'int'))) or _error("SQL_ERROR_THROWEN");
    ...
    public function connect($do, $id, $uid = null, $value = null)
    {
        ...
        switch ($do) {
            ...
            case 'friend-accept':
                ...
                $response = shntrToken::payRelysia($friendRequestAcceptReward, $this->_data['user_relysia_address'], 0);
                shntrToken::noteTransaction(
            ...
            case 'friend-add':
                ...
                $query = $db->query(
                    'select user_relysia_address as address, user_id as id from users where user_id = 1 limit 1'
                ) or _error("SQL_ERROR_THROWEN");
                $superUser = $query->fetch_assoc();
            ...
            case 'friend-fund':
                $query = $db->query(sprintf('select user_relysia_address as address from users where user_id = %1$s limit 1', secure($id, 'int'))) or _error("SQL_ERROR_THROWEN");
                $recipientAddress = $query->fetch_row()[0];
    ...
    public function breach_paywall(int $user_id): void
    {
        ...
        if ($this->_logged_in) {
            ...
            $query = $db->query(sprintf('select user_relysia_address as address from users where user_id = %1$s limit 1', secure($user_id, 'int'))) or _error("SQL_ERROR_THROWEN");
            $recipientAddress = $query->fetch_row()[0];
    ...
    public function user_reached_daily_chat_limit()
    {
        global $db, $system;

        $counter = $db->query(
            sprintf(
                'select count(1) as message_counter from conversations_messages where user_id = %s and `time` > now() - interval 1 day',
                secure($this->_data['user_id'], 'int')
            )
        )->fetch_assoc();

        return $system['max_daily_chat_messages'] <= $counter['message_counter'];
    }
    ...
    public function publisher($args = [])
    {
        ...
        try {
            ...
            if ($post['post_type'] == 'product') {
                ...
                $superUser = $query->fetch_assoc();
                shntrToken::payRelysia(100, shntrToken::getshntrTreasure('address'), $this->_data['user_id']);
                shntrToken::noteTransaction(
                    100,
                    intval($this->_data['user_id']),
                    intval($superUser['id']),
    ...
    public function register_to_relysia(string $username, int $user_id): void
    {
        ...
        shntrToken::payRelysia(1000, $address, 0);
    }
