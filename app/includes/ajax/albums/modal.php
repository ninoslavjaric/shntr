...
try {
	...
	switch ($_GET['do']) {
		case 'edit_title':
			...
			$return['callback'] = "$('#modal').modal('show'); $('#modal').removeClass('blue-modal'); $('#modal .modal-content:last').html(response.edit_title);";
			break;
		...
		case 'add_photos':
			...
			$return['callback'] = "$('#modal').modal('show'); $('#modal').removeClass('blue-modal'); $('#modal .modal-content:last').html(response.add_photos); initialize_modal();";
			break;
