...
$(function () {
    ...
    function _post_message(element) {
        ...
        $.post(api['chat/post'], data, function (response) {
            ...
            /* check if there is a callback */
            if (response.callback) {
                if (response.callback.includes('error') && _guid) {
                    const messageBlock = document.getElementById(_guid);
                    messageBlock.style.opacity = '.5';
                    messageBlock.querySelector('.text').style.background = 'red';
                }
                eval(response.callback);
            } else {
                if (widget.hasClass('fresh')) {
